...
coarseRes = 100; % grid size in coarse grid
...
save('B:\TDOAmodel_100m', 'TDOA', 'wloc')
...
for nCoarse = 1:length(wlocCoarse)
    ...
    xvecFine = (wlocCoarse(nCoarse, 1)-3*coarseRes):fineRes:(wlocCoarse(nCoarse, 1)+3*coarseRes);
    yvecFine = (wlocCoarse(nCoarse, 2)-3*coarseRes):fineRes:(wlocCoarse(nCoarse, 2)+3*coarseRes);
    zvecFine = (wlocCoarse(nCoarse, 3)-3*coarseRes):fineRes:(wlocCoarse(nCoarse, 3)+3*coarseRes);
    ...
    save(['B:\modelFiles_10mFrom100m\TDOAmodel_10m_n=', sprintf('%05d', nCoarse)], 'TDOA', 'wloc')
end

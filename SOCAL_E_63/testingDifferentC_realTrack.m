...
for mi = 1:numel(M)
    [M{mi}.TDOA, M{mi}.wloc] = makeModel(-600:25:500, -2500:50:500, 0:20:800, hloc, H1, H2, M{mi}.c);
end
%%
global LOC
...
Iuse = find(sum(~isnan(whale{1}.TDOA), 2)==18);

for i = 1:length(Iuse)

    Isml = find(~isnan(whale{1}.TDOA(Iuse(i), 1:12)));
    Ilrg = find(~isnan(whale{1}.TDOA(Iuse(i), 13:end)));
    
    Asml = (2*pi*sig2sml)^(-length(Isml)/2); % coefficient of small ap
    Alrg = (2*pi*sig2lrg)^(-length(Ilrg)/2); % coefficient of large ap
    for mi = 1:numel(M)
        tdoasml = whale{1}.TDOA(i, Isml);
        tdoalrg = whale{1}.TDOA(i, Ilrg+12) - drft(Ilrg);
        Lsml = Asml*exp(-1./(2.*sig2sml).*sum((M{mi}.TDOA(:,Isml)-tdoasml).^2, 2));
        Llrg = Alrg.*exp(-1./(2.*sig2lrg).*sum((M{mi}.TDOA(:,Ilrg+12)-tdoalrg).^2, 2));
        
%         L = Lsml.*Llrg;
        L = Llrg;

        [R{mi}.Lmax(i), Imax] = max(L);
        R{mi}.wloc(i, :) = M{mi}.wloc(Imax, :);
        R{mi}.Imax(i) = Imax;
    end
end

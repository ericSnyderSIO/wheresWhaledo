...
for wn = 1:numel(whale)
    if ~isempty(whale{wn})
        ...
        Iuse = find(sum(~isnan(whale{wn}.TDOA), 2)>=7);

        for ndet = 1:length(Iuse)
            i = i+1;
            TDet(i) = whale{wn}.TDet(Iuse(ndet));
            tdoameas = whale{wn}.TDOA(Iuse(ndet),:);
            wloc = whale{wn}.wloc(Iuse(ndet), :);
            [tdoaexp] = calcTDOAerr_1det(wloc, hloc, H1, H2, c);
...
function [TDOAexp] = calcTDOAerr_1det(wloc, hloc, H1, H2, c)

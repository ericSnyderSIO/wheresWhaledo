...
tdoaDir = dir(['*TDOA*', trackName, '*.mat']);
load(tdoaDir.name)

% load other detections
detDir = dir(['*det*', trackName, '*.mat']); 
load(detDir.name)
...
%     14, (brushing.params.colorMat(wn+2, :).'.*(1-.5.*(whaleLoc_DOA{wn}.ti-whaleLoc_DOA{wn}.ti(1))./max(whaleLoc_DOA{wn}.ti-whaleLoc_DOA{wn}.ti(1)))).', 'filled')
...
for wn = 1:numel(whale)
% wn = 3
p(2) = scatter3(whale{wn}.wloc_fine(:, 1)+h1(1), whale{wn}.wloc_fine(:, 2)+h1(2), whale{wn}.wloc_fine(:, 3)+h1(3), ...
...
ylabel('N-S')
zlabel('height above arrays')
% legend(p, 'DOA intersect', 'TDOA model')

save(trackName, 'whale')